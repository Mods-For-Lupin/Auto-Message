...
public class ServerMessageService {
  ...
  public static void load() {
    ...
    File configFile = new File(
        configDirPath + File.separator + Constants.MOD_ID + "-server" + ".json");
    ...
    if (!configFile.isFile()) {
      ...
      File configMessagesFile = new File(
          configDirPath + File.separator + "messages" + File.separator + "server_messages.json");
      MessageServiceUtil.copyResourceToFile("/assets/automessage/server_messages.json",
          configMessagesFile);
      load();
      return;
    }
    ...
    if (configFile.isFile()) {
      ...
      if (instance.message_json_files == null || instance.message_json_files.isEmpty()) {
        Constants.LOG.info("[SERVER] No message files listed in configuration: {}",
            configFile.getName());
      ...
      for (String pattern : instance.message_json_files) {
        List<File> filesToLoad =
            pattern.contains("*") ? MessageServiceUtil.resolveMessagePaths(pattern)
                : List.of(new File("config", pattern));
...
//            }
    ...
    long time = end - start;
    Constants.LOG.info("[SERVER] MessageService loaded on server in {}ns or {}ms or {}s", time,
        time / 1_000_000L, (float) time / (1_000_000.0f * 1_000.0f));
  }
